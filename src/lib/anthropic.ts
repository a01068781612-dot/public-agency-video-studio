...
export async function generateScript(params: {
  ...
  const user = [
    ...
    '- ★단조로움 방지★ bullets 는 전체 중간 씬의 40%를 넘기지 마라(직전 영상은 절반이 bullets라 밋밋했다). diagram·comparison·code·quote 를 각각 최소 1회 이상 반드시 쓰고, 같은 타입이 세 씬 연속으로 오지 않게 배치한다.',
